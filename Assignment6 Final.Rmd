...
##Answer: We would like to choose cluster as an indicator to predict the score of jumping rope. By observing the number of jumps for different clusters in Task2_outcomes, we could find that people in cluster4 have better performance on average. According to p2 we could figure out that group members in cluster 4 have least movement. So, there is an inverse relationship bwtween number of jumps and movements during jumping, and it's safe to make a conclusion that in order to jump more in the limited time, expert would keep the least level of movements to save both energy and time. Therefore, we would like to assign higher scores for group members in cluster 4. On the other side, members in cluster 1 and 3 have relative larger movement, so we will assign lower scores for them. 

## Our advice would be that try to make less movement during jumping rope. Don't jump to high and try to jump in the same place.
